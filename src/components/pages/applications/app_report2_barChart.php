<?php
    ...
    $left_selected = "APPREPORT2";
    $tabTitle = "APP Report 2";
    ...
    <meta name="viewport" content="width=device-width, initial-scale=1">
        ...
        function createBarChart(barChart){
            ...
            switch(name){
                ...
                case 'Component':
                    <?php
                    $query = $db->query("SELECT requester, COUNT(requester) AS occurrences FROM (SELECT DISTINCT cmpt_name, cmpt_version, requester from apps_components) as subquery GROUP BY requester;");
                    while($query_row = $query->fetch_assoc()) {
                        echo 'queryArray.push(["'.$query_row["requester"].'", '.$query_row["occurrences"].', "'.$query_row["requester"].'"]);';
                    }
                    ?>
                    break;
